...
void handle_client(int client_fd) {
    ...
    while (true) {
        ...
        if (cmd == CMD_SEND_MATRIX) {
            ...
            status = 1; 
            send_ack(client_fd); // кажемо клієнту про початок
            ...
            uint32_t status_cmd = htonl(CMD_STATUS);
            uint32_t status_len = htonl(4);
            uint32_t status_val = htonl((uint32_t)status.load());
            write_n(client_fd, &status_cmd, 4);
            write_n(client_fd, &status_len, 4);
            write_n(client_fd, &status_val, 4);
            ...
            if (status != 2) {
                ...
                uint32_t result_cmd = htonl(CMD_RESULT);
                uint32_t message_len = 4 + 4 + n * n * 4;
                uint32_t result_len = htonl(message_len);
                uint32_t result_ms = htonl(elapsed_ms);
                uint32_t result_n = htonl(n);

                write_n(client_fd, &result_cmd, 4);
                write_n(client_fd, &result_len, 4);
                write_n(client_fd, &result_ms, 4);
                write_n(client_fd, &result_n, 4);
                
                for (int32_t val : result_matrix) {
                    int32_t val_net = htonl(val);
                    write_n(client_fd, &val_net, 4);
                }
